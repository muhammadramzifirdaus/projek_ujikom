        ...
        <div class="p-5 border-b border-gray-200 bg-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
                <form action="{{ route("admin.alat.index") }}" method="GET" class="flex w-full md:w-80">
                    ...
                    @if(request("search"))
                        <a href="{{ route("admin.alat.index") }}" class="ml-2 bg-gray-300 hover:bg-gray-400 text-gray-700 px-3 py-2 text-sm rounded-lg flex items-center transition">
                            Reset
                        </a>
                    @endif
                </form>
                ...
                <tbody class="text-gray-700 text-sm">
                    @forelse($alats as $alat)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="py-3 px-4 border-b">
                                <span class="px-2.5 py-1 text-xs font-semibold rounded-full {{ strtolower($alat->status_kondisi) == "baik" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800" }}">
                                    {{ $alat->status_kondisi }}
                                </span>
                            </td>
                            <td class="py-3 px-4 border-b">
