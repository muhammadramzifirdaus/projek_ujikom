...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200">
        <div class="p-5 border-b border-gray-200 bg-gray-50 flex flex-col gap-3">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-gray-700 text-sm">
                    @forelse($peminjamans as $item)
                        <tr class="hover:bg-gray-50 transition align-top">
                            ...
                            <td class="py-3 px-4 border-b">
                                <span class="px-2.5 py-1 text-xs font-semibold rounded-full {{ $item->status == 'telat' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800' }}">
                                    {{ ucfirst($item->status) }}
                                </span>
                            </td>
                            <td class="py-3 px-4 border-b">
                            ...
                            <td class="py-3 px-4 border-b">
                                <form action="{{ route('petugas.pengembalian.proses', $item->id) }}" method="POST"
                                    class="bg-gray-50 p-3 rounded-lg border border-gray-200 text-left space-y-2">
                                    @csrf
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 mb-1">Kondisi Kembali</label>
                                        <select name="kondisi_kembali" required class="w-full text-xs border border-gray-300 rounded p-1.5 focus:ring-emerald-500 focus:border-emerald-500">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="denda" value="0" placeholder="0"
                                            class="w-full text-xs border border-gray-300 rounded p-1.5 focus:ring-emerald-500 focus:border-emerald-500">
                                    </div>
                                    <button type="submit" onclick="return confirm('Proses pengembalian alat ini?')"
                                        class="w-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs py-1.5 px-3 rounded font-semibold transition shadow-sm">
                                        Proses Pengembalian
                                    </button>
                                </form>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-6 text-center text-gray-500">Tidak ada peminjaman yang sedang aktif.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
